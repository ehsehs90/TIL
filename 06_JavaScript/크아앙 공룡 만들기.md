...
- `<script>` 태그 추가 (callback_intro.html)

  - '클릭해주세요' 버튼을 누르면 console 창에 '버튼이 클릭됐어요' 이 출력된다.
  ...
  ![image-20191121112748773](assets/image-20191121112748773.png)
...
2. 익명 함수인 콜백 함수 추가
   	...
   	- 먼저 doSomething 이 수행 된 후, 익명함수로 정의한 콜백함수가 수행된다.
...
3. 기명 함수인 콜백 함수로 변경해서 추가
   ...
   - 우리가 원하는 시점에 함수를 호출해서 사용할 수 있게 된다
...
- JavaScript 를 이용해서 요소를 선택해서 객체로 반환하기
...
- 변수 할당하기
  ...
  - `.bg`라는 클래스 태그 , 그 자식태그까지 전부 변수에 담는다.
...
- 요소가 가지고 있는`remove`함수를 통해 해당 요소를 삭제할 수 있다.
  ...
  ```javascript
  coconst dino = document.querySelector('#dino')
  dino.remove()
  ```
...
- 새롭게 생성한 태그는 어떠한 속성이나 값이 없는 상태이기 때문에 일일이 값을 할당해 주어야 한다.

#### 3.5.4 대상에 요소 추가하기

- 단순한 태그 생성만으오는 새로운 요소가 DOM 에 자동으로 렌더링되지는 않는다.

- 삽입하고 싶은 위치에서 부모 요소의 `append`함수를 통해 가장 뒤에 추가해주거나, 

- `insertBefore`함수를 통해 자식 요소 중 특정 요소 전에 삽입할 수 있다.
...
- 요소에 이벤트를 등록하여 해당 이벤트가 발생할 때마다 설정한 함수가 실행되도록 한다.
- 이벤트(Event) : 어떤 사건
  - 사용자가 버튼 클릭했을 때, 웹 페이지가 정상적으로 로드 되었을 때 등
...
> dino의 style 에 margin 값을 부여하면..? -> `dino.style.marginRight='20px'`

- [문제점] 한 번만 움직이고 더이상 움직이지 않는다.

  - margin 값을 더해주는것이 아니라, 똑같이 20px 이라는 값을 재할당 하고 있기 때문이다
  ...
  ```javaScript
    document.addEventListener('keydown', function(e){
        ...
        if (e.code == 'ArrowLeft'){
          ...
          console.log('ArrowDown')
          dino.style.marginUp ='20px'
        }else if (e.code == 'ArrowUp'){
          console.log('ArrowUp')        
          dino.style.marginDown ='20px'
        }else if (e.code == 'ArrowRight'){
          console.log('ArrowRight')        
...
> x와  y변수에 기본값 0 을 할당하고, 방향키가 입력될 때마다, margin 값을 계속해서 더하고 빼는 과정을 통해 공룡의 위치를 이동시키자
