...
### Foreign Key
 ...
 - 참조 키의 값으로는 부모 테이블에 존재하는 키의 값만을 넣을 수 있다
   ...
   참조 무결성을 위해 참조 키를 사용하여 부모 테이블의 유일한 값(PK)을 참조한다

   - 부모 테이블의 기본 키를 참조

- 참조 키의 값이 부모 테입르의 기본 키일 필요는 없지만 유일해야한다.
...
- Foreign Key
  ...
  - 다시 migrate
    ...
    - 'Detault 값' 설정 안내가 나온다

    - 만약 데이터가 있었다면 어떤 값으로 할래?

      ![image-20191116215507669](Django 댓글 CRUD.assets/image-20191116215507669.png)

    - migrate 후 sqlite3 확인

    - ![image-20191116215613554](Django 댓글 CRUD.assets/image-20191116215613554.png)

## ORM실습

> shell_plus 를 이용해 댓글을 생성하고 조회해 보자.

### 2.1 댓글 생성(댓글 인스턴스 하드코딩으로 만들기)
